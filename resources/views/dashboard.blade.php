    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {{-- CUSTOMER DASHBOARD --}}
                @if(Auth::user()->role === 'customer')
                    <x-dashboard-card title="Browse All Products" color="blue" icon="tv" link="{{ route('products') }}">
                        Explore all electronics available in the store.
                    </x-dashboard-card>

                    <x-dashboard-card title="Browse by Category" color="blue" icon="tablet-alt" link="{{ route('product.category', 1) }}">
                        Filter electronics by category.
                    </x-dashboard-card>

                    <x-dashboard-card title="Checkout" color="blue" icon="credit-card" link="{{ route('cart.index') }}">
                        Complete your purchase for selected products.
                    </x-dashboard-card>

                    <x-dashboard-card title="Transaction History" color="blue" icon="receipt" link="{{ route('transaction.history') }}">
